...
Precision curve-aligned raymarching through 4D space
1:1 mathematical alignment: smoothstep, sin, cos, tan, hermite, bezier
"""
...
class Curves:
    """Exact mathematical curves for epsilon modulation"""
    ...
    def bezier3(t: np.ndarray, p0: float, p1: float, p2: float, p3: float) -> np.ndarray:
        """Cubic Bezier: Bernstein polynomial basis"""
        mt = 1.0 - t
        return mt**3*p0 + 3*mt**2*t*p1 + 3*mt*t**2*p2 + t**3*p3
    
    @staticmethod
    def exp_decay(x: np.ndarray, rate: float = 1.0) -> np.ndarray:
        """Exponential falloff: e^(-x*rate)"""
    ...
    def quantum_interference(x: np.ndarray, n_waves: int = 4) -> np.ndarray:
        """Wave superposition with phase interference"""
        result = np.zeros_like(x)
        for i in range(n_waves):
            phase = i * np.pi / n_waves
            amp = 1.0 / (i + 1)
            result += amp * np.sin(x * (i + 1) * np.pi + phase)
...
class SDF4D:
    ...
    def duocylinder(p: np.ndarray, r1: float = 0.8, r2: float = 0.8) -> np.ndarray:
        """Unique 4D shape - Cartesian product of two circles"""
...
class VolumetricEpsilonCaster:
    ...
    def _adaptive_epsilon(self, t: np.ndarray, step: int) -> np.ndarray:
        ...
        - Step count (more steps = slightly larger for convergence)
        """
        dist_scale = 1.0 + t * 0.03
        step_factor = Curves.smoothstep(0, self.max_steps, 
                                        np.full_like(t, step))
        return self.epsilon_base * dist_scale * (1.0 + step_factor * 0.4)
    ...
    def cast(self, sdf: Callable, w_slice: float = 0.0,
        ...
        for step in range(self.max_steps):
            ...
            # Curve-modulated step with bezier relaxation
            relax = Curves.bezier3(step / self.max_steps, 0.8, 0.85, 0.95, 1.0)
        ...
        if volumetric:
            density, q_field = self._volumetric_pass(
                sdf, origins, dirs, R_inv, self.max_dist)
...
def main():
    ...
    print("    ├─ base: 0.0006")
    print("    ├─ dist_scale: 1.0 + t * 0.03")
    print("    └─ step_factor: smoothstep(0, max_steps, step)")
    print()
    print("  CURVE LIBRARY:")
    print("    ├─ smoothstep    → t²(3-2t)")
    print("    ├─ smootherstep  → t³(t(6t-15)+10)")
    print("    ├─ hermite       → cubic spline")
    print("    ├─ bezier3       → cubic Bezier")
    print("    ├─ sin/cos       → phase-aligned")
    print("    ├─ exp_decay     → e^(-x·rate)")
    print("    ├─ smin/smax     → smooth boolean")
    print("    └─ quantum_interference → n-wave superposition")
